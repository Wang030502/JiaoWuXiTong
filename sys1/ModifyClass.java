...
public class ModifyClass extends JDialog implements ActionListener {
    ...
    JTable jtb;
    JScrollPane jsp;
    public static int i = 0;//i为1的时候确认了修改课程，i为0则取消修改
    ...
    public void addView() {
        ...
        //小图标
        ImageIcon bi1 = new ImageIcon("E:\\Java(IDEA)\\IntelliJ IDEA Community Edition 2021.3.2\\SYSTEM\\system\\src\\images2\\退出.png");
    ...
    public void actionPerformed(ActionEvent arg0) {
        if (arg0.getSource() == jb1) {
            ...
            if (rowNo == -1) {
                ...
                //int Remain2 = (Integer) tm.getValueAt(rowNo,5);显示异常
                String Remain = (String) tm.getValueAt(rowNo, 5);//某一行的第5列就是剩余数
                int s = Integer.parseInt(Remain);//需要转型因为，你退课了剩余数要加1
                ...
                int s2 = Integer.parseInt(Source);//也要转型，在数据库里设置的为int
                String All = (String) tm.getValueAt(rowNo, 6);
                int s3 = Integer.parseInt(All);//也要转型，在数据库里设置的为int

                Object[] options = {"确定", "取消"};
                int respose = JOptionPane.showOptionDialog(this, "确认退出该课程吗？", "提示", JOptionPane.DEFAULT_OPTION, 1, null, options, options[0]);
                if (respose == 0) {
                    i = 1;
                    //连接数据库
